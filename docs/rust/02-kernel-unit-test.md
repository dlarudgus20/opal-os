...
- `.ktest`는 Rust kernel test 등록 영역입니다.
- Rust unit-test 빌드는 `build/unit-test/<platform>/<config>`에 산출물을 만들어 일반 `build/<platform>/<config>` 산출물과 분리합니다.
- 적용 범위는 Rust unit-test 모드뿐입니다.

## 2. 실행 흐름
1. `make RUST=1 unit-test`가 `UNIT_TEST=1`로 Rust 커널을 빌드합니다.
2. `rules-rust.mk`가 `build/unit-test/<platform>/<config>`와 `--cfg opal_ktest`를 적용합니다.
3. `opal-kernel::kmain()`은 `opal_ktest` cfg에서 `ktest::run()`을 호출합니다.
...
테스트 함수는 `opal-kernel` crate 어디에 있어도 됩니다.
일반 빌드에서는 `#[ktest]`가 붙은 함수와 등록 static이 `#[cfg(opal_ktest)]`로 제외됩니다.
