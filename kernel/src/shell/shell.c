...
#define CMD_ARGV(name, help, fn) { (name), (help), true, { .argv_handler = (fn) } }
    ...
    CMD_ARGV("mount",       "mount fs: mount [fstype] [bdev]",      shell_cmd_mount),
    CMD_ARGV("mkfs",        "format fs: mkfs [fstype] [bdev]",      shell_cmd_mkfs),
    CMD_ARGV("cat",         "cat /path | cat > /path",              shell_cmd_cat),
    CMD_ARGV("ls",          "list entries: ls [path]",              shell_cmd_ls),
    CMD_ARGV("mkdir",       "make directory: mkdir /path",          shell_cmd_mkdir),
