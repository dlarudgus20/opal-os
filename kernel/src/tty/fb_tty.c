...
static void draw_cursor(struct fb_tty *tty) {
    fb_fill_rect(
        tty->xoff + tty->xcur * CW + MARGIN, tty->yoff + (tty->ycur + 1) * CH + MARGIN - CUR_THICK,
        CW, CUR_THICK,
        tty->fg
    );
}
...
static void clear_screen(struct fb_tty *tty) {
    ...
    fb_draw_rect(
        tty->xoff + offset, tty->yoff + offset,
        tty->width * CW + (MARGIN - offset) * 2, tty->height * CH + (MARGIN - offset) * 2,
        BORDER, g_colors[BORDER_COLOR]
    );
...
static void scroll(struct fb_tty *tty) {
    if (tty->xcur < tty->width) {
        fb_fill_rect(
            tty->xoff + tty->xcur * CW + MARGIN, tty->yoff + tty->ycur * CH + MARGIN,
            CW, CH,
            tty->bg
        );
    }
    ...
    fb_bitblt(
        tty->xoff + MARGIN, tty->yoff + MARGIN,
        tty->width * CW, (tty->height - 1) * CH,
        tty->xoff + MARGIN, tty->yoff + CH + MARGIN
    );
    fb_fill_rect(
        tty->xoff + MARGIN, tty->yoff + (tty->height - 1) * CH + MARGIN,
        tty->width * CW, CH,
        tty->bg
    );
    draw_cursor(tty);
